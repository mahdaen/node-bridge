    ...
    if ( pkgs[ name ] ) {
        ...
        if ( version ) {
            ...
            if ( pkgn[ version ] ) {
                ...
                var removed = PackageRemoveVersion(name, version, auto, force);

                /* Removing it self */
                if ( removed ) {
                    PackageRemoveSelf(name, auto, force);
                }
            }
            else {
...
function PackageRemoveVersion ( name, version, auto, force ) {
    ...
    DependentRemover(name, version, pkgv);
    ...
    if ( deps.length > 0 && 'string' === typeof auto ) {
        ...
        loop(pkgv.dependencies, function ( dn, dv ) {
            ...
            if ( sbm ) {
                ...
                PackageRemover(dn, sbm.version, auto, force);
            }
...
function DependentRemover ( name, version, pkg, target, save ) {
    ...
    if ( pkg.dependencies ) {
        loop(pkg.dependencies, function ( dname, dvers ) {
            var dpkg = ModuleFinder(dname, dvers, true, true);
