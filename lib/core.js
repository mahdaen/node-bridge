...
if ( osp === 'windows' ) {
    nbrpath = path.resolve(oses.homedir(), 'appdata/roaming');
}
else {
    nbrpath = path.resolve('/usr/local');
}

/* Getting Required Variables */
var cwd = process.cwd(),
    reg = path.resolve(nbrpath, 'node-bridge'),
    tmp = path.resolve(reg, 'tmp'),
    bin = path.resolve(reg, 'bin'),
...
function initRoot () {
    try {
        ...
        file.ensureDirSync(bin);
    }
    catch ( err ) {
        console.log(`${colr.redBright('EACCES: Permission denied!')}`);
        console.log('If this is the first time you use npm-bridge, you need to run as root to grant access.');
        console.log('If you are a windows user, you need to always run npm-bridge as administrator.');
        process.exit();
    }

    try {
        if ( osp === 'windows' ) {
            bash.spawnSync('icacls', [ reg, '/T', '/inheritance:e', '/grant', 'everyone:F' ]);
        }
        else {
...
function PackageInstaller ( name, version, owner, force ) {
    ...
    loop(submod, function ( pkgp ) {
        ...
        if ( bpkg ) {
            ...
            /* Add bin symlink */
            if ( bpkg.bin ) {
                loop(bpkg.bin, function ( bnam, bsrc ) {
                    var srcpath = path.resolve(mwd, base, bpkg.version, bsrc),
                        despath = path.resolve(mwd, '.bin', bnam);

                    try {
                        WriteBin(despath, bsrc);
                        //MakeSymlink(srcpath, despath);
                    }
                    catch ( err ) {}
            ...
            if ( dpkg ) {
                ...
                if ( dpkg.bin ) {
                    loop(dpkg.bin, function ( des, src ) {
                        WriteBin(path.resolve(nmpath, '.bin', des), path.resolve(dpkg.path, 'src'));
                        //MakeSymlink(path.resolve(dpkg.path, src), path.resolve(nmpath, '.bin', des));
                    });
                }

                // Create the link.
                MakeSymlink(dpkg.path, path.resolve(nmpath, dpkg.name));
            }
...
function MakeSymlink ( src, des ) {
    try {
        file.ensureSymlinkSync(src, des);
    }
    catch ( err ) {
        console.log(`${colr.redBright('EPERM: operation not permitted!')}`);
        console.log('If you\'re a windows user, you need to always run npm-bridge as administrator.');
...
function WriteBin ( des, src ) {
    var cmd = file.readFileSync('../tpl/bin-cmd', 'utf8'),
        jse = file.readFileSync('../tpl/bin-jse', 'utf8');

    if ( osp === 'windows' ) {
        cmd = cmd.replace(new RegExp('%%BINPATH%%', 'g'), src);
        jse = jse.replace(new RegExp('%%BINPATH%%', 'g'), src);

        file.writeFileSync(des, jse);
        file.writeFileSync(`${des}.cmd`, jse);
    }
    else {
        MakeSymlink(src, des);
    }
