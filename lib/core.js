...
function PackageRemover ( name, version, auto ) {
    ...
    if ( pkgs[ name ] ) {
        ...
        if ( version ) {
            console.log(`${auto || ''}${colr.bold('Removing')} ${name}@${version}`);

            /* Continue if version is installed */
            if ( pkgn[ version ] ) {
                /* Removing each version */
                remVersion(name, version, auto);
        ...
        /* Using unversioned removal */
            ...
            loop(pkgn, function ( version, defs ) {
                console.log(`${colr.bold('Removing')} ${name}@${version}`);
...
/* Remove each version */
function remVersion ( name, version, auto ) {
