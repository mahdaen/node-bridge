...
if ( osp === 'windows' ) {
    nbrpath = path.resolve('/program files');
}
else {
...
if ( osp === 'windows' ) {
    binpath = path.resolve(oses.homedir(), 'appdata/roaming/node-bridge/bin');
}
else if ( osp === 'linux' ) {
    binpath = path.resolve('/usr/bin');
}
else if ( osp === 'darwin' ) {
    binpath = path.resolve('/usr/local/bin');
}
...
function initRoot () {
    ...
    try {
        if ( osp === 'windows' ) {
            ...
            bash.spawnSync('chown', [ '-R', '$(whoami)', reg ]);
        }
    }
    catch ( err ) {
    ...
    try {
        if ( osp === 'windows' ) {
            bash.spawnSync('icacls', [ bin, '/T', '/inheritance:e', '/grant', 'everyone:F' ]);
        }
        else {
            bash.spawnSync('chown', [ '-R', '$(whoami)', bin ]);
        }
    }
    catch ( err ) {
        console.log(err.message);
