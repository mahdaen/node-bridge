...
/* Package Installer */
function PackageInstaller ( name, version, owner ) {
...
function PackageLinker ( pkgdef, deep, fn ) {
    ...
    /* Add bin symlink */
    if ( pkgdef.bin ) {
        loop(pkgdef.bin, function ( des, src ) {
            file.ensureLinkSync(path.resolve(pkgdef.path, src), path.resolve(nmpath, '.bin', des));
        });
    }
}
