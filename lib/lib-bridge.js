...
class Bridge {
    ...
    // Update checker.
    static getupd ( name, version, scope ) {
        var $this = this;

        // Create name by joining name and version.
        name = `${name}@${version}`;

        try {
            // Check for updates by getting package info via npm.
            var pkg = $exec(`npm${$this.ext}`, [ 'info', name ], { cwd : scope.twd });

            // If result status > 0, mark as error.
            if ( pkg.status > 0 ) {
                $logs(pkg.stderr.toString());
            }
            else {
                // Convert NPM result to object.
                eval('pkg = ' + pkg.stdout.toString());
            ...
            if ( optis('debg') ) $logs(err.message);
            throw err;
        }
    }
    ...
    static rmdir ( dirs ) {
        if ( 'string' === typeof dirs ) {
            try {
                ...
                if ( self.optis('debg') ) $logs(err.message);
     ...
     * Binary Linker
     * This static method using file.ensureSymlinkSync from fs-extra.
     * Before perfom linking, this method should check which platform used,
     * sice on windows platform we need to create script to run package binaries.
     *
     * @param src       // Source path, actual path where package binary live.
     * @param des       // Destination path, the path where binary will save to.
     * @returns {*}     Undefined for success, error object if some error happened.
     */
    static linkbin ( src, des ) {
        if ( 'string' === typeof src && 'string' === typeof des ) {
            try {
                if ( cli.osp === 'windows' ) {
                    let cmd = _readfile(resolve(__dirname, '../tpl/bin-cmd'), 'utf8'),
                        jse = _readfile(resolve(__dirname, '../tpl/bin-jse'), 'utf8');

                    cmd = cmd.replace(new RegExp('%%BINPATH%%', 'g'), src);
                    jse = jse.replace(new RegExp('%%BINPATH%%', 'g'), src);

                    _writefile(`${des}.cmd`, cmd);
                    _writefile(des, jse);
                }
                else {
                    self.linkdir(src, des);
                }
            }
            catch ( err ) {
                if ( self.optis('debg') ) $logs(err.message);
        ...
        if ( 'string' === typeof cmd && Array.isArray(args) ) {
            ...
            loop(cmd, function ( cmdo ) {
                if ( 'object' === typeof cmdo && 'cmd' in cmdo && 'string' === typeof cmdo.cmd && 'args' in cmdo && Array.isArray(cmdo.args) ) {
                    ...
                    let rsl = self.spawn(cmdo.cmd, cmdo.args, cwd);
